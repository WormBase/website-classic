...
# Author: T. Harris, 2 November 2004. Go Kerry!
# This script interpolates genetic positions onto the physical map.
# Currently, this includes two-and-three factor mapping experiments

# It also generates a GFF file for loading into a Bio::DB::GFF database,
# with one entry for every mapping experiment
...
if ($help) {
  die <<END;
 Usage: interpolate_gmap_to_pmap.pl [options]
...
#open GFF,  ">/usr/local/wormbase/temporary_patches/${version}_patches/$version.gmap-to-pmap.gff";
...
#      }
    ...
    print join("\t",$chrom,$source,'gmap_span',$fpmap_lower,$fpmap_upper,'.','.','.',
