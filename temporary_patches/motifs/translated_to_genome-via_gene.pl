...
my ($acedb,$host,$port,$help,$debug,$generate_gff,$format,$test,$filter,$version);
...
if ($help) {
 ...
 Usage: interpolate_pmap_to_gmap.pl [options]
  ...
  -filter   Filter duplicate motifs by sequence (will also change CDS to public_name)
...
#open GFF,">/usr/local/wormbase/temporary_patches/$version_patches/$version.motifs.gff" if $generate_gff;
open DEBUG,">/usr/local/wormbase/temporary_patches/${version}_patches/$version.motifs.debug" if $debug;
open CONFIRM,">/usr/local/wormbase/temporary_patches/${version}_patches/$version.motifs.protein_confirmation";
...
while (my $gene = $i->next) {
  ...
  foreach my $cds (@cds) {
	...
	if ($filter) { # Filter out duplicate motifs based on amino acid sequence
	  my $subseq = substr($protein_seq,$motif_start,($motif_stop - $motif_start));
	  if (defined $seen->{$type}->{$subseq}) {
	    next;
	  }
	  $seen->{$type}->{$subseq} = $cds;
	}
	$id++;
...
sub generate_gff {
  my ($data,$aa2nt) = @_;
  my ($refseq,$protein,$cds,$type,$start,$stop,$mstart,$mstop,$strand,$score,$id,$exons_covered,$desc,$public_name) =
    map { $data->{$_} } qw/chrom protein cds type start stop aa_start aa_stop strand score id exons_covered desc public_name/;
  $strand ||= "+";
  $score  ||= SCORE;
  ...
  my $group = ($filter) ? qq{Motif "$public_name-$type.$id"} : qq{Motif "$protein-$type.$id"};
  $group =~ s/\:/_/g;
  my $full_group = qq{$group; Note "CDS=$cds" ; Note "Type=$type" ; Note "Range=$aarange" ; Note "Exons=$exons_covered" ; Note "Protein=$protein"};
...
sub print_gff {
  ...
  ($start,$stop) = ($stop,$start) if ($start > $stop);
  print join("\t",$ref,$source,$method,$start,$stop,@rest),"\n";
}
...
#  my @exons     = grep { $_->name eq $cds } $seq_obj->features('exon:curated') if $seq_obj;
  ...
  return unless $gene;
  my @exons = grep { $_->name eq $cds } $gene->features('coding_exon:curated') if $gene;
