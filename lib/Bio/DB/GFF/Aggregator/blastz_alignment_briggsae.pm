...
Bio::DB::GFF::Aggregator::blastz_alignment -- A WABA alignment
...
=head1 SYNOPSIS
  ...
  my $db      = Bio::DB::GFF->new( -adaptor => 'dbi:mysql',
                                   -dsn     => 'dbi:mysql:briggsae42',
				   -aggregator => ['blastz_alignment'],
				 );
  # fetch the synthetic feature type "blastz_alignment"
  my @blastz    = $db->features('waba_alignment');

 -------------------------------------------------------------------------------------
 Aggregator method: blastz_alignment
 Main method:       -none
 Sub methods:       nucleotide_match_briggsae:blastz
 -------------------------------------------------------------------------------------

=head1 DESCRIPTION

Bio::DB::GFF::Aggregator::blastz_alignment handles the type of
alignments produced by Jim Kent's WABA program, and was written to be
compatible with the C briggsae GFF files.  It aggregates the following
feature types into an aggregate type of "blastz_alignment":

   nucleotide_match_briggsae:blastz

=cut

package Bio::DB::GFF::Aggregator::blastz_alignment_briggsae;
...
=head2 method
 ...
 Returns : the string "blastz_alignment"
 Args    : none
 Status  : Public

=cut

sub method { 'blastz_alignment_briggsae' }
...
=head2 part_names
 ...
 Returns : the list "nucleotide_match_briggsae:blastz"
 Args    : none
 Status  : Public
...
sub aggregate {
  ...
  foreach (keys %aggregates) {
    ...
    $base->source('blastz') if $pseudo_method eq $meth;
