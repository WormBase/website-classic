...
WormBase runs on a Unix or Linux system.  A relatively fast system
with generous memory is strongly recommended.  The minimum suggested
hardware is:

=over 4

=item 900 Mhz Pentium III or higher

=item 4 gigabyte RAM

=item 4 gigabyte Swap

=item 20 gigabytes free disk space

=back

Each database occupies approximately 5 gigabytes of disk space, and
...
gigabyte used by BLAST databases.

WormBase is currently in a transition between an ACeDB-based system
...
=item acedb version 4.8j or higher

	http://www.acedb.org/

=item Perl version 5.6.1 or higher

	http://www.perl.org/

=item The following Perl modules, all of which are available at
...
will be able to produce output in the Scalable Vector Graphics format.

=item Apache 1.3.26 or higher

	http://www.apache.org/

=item Mod_Perl 1.27 or higher

	http://perl.apache.org/

=item Expat 1.95.1 or higher

	http://expat.sourceforge.net/

=item The WormBase Web software, matched to the data release version

        ftp://www.wormbase.org/pub/wormbase/software or via CVS

=item The Generic Genome Browser package, version 1.61 or higher

        http://www.gmod.org/

=item The BioPerl package, version 1.40 or higher

        http://www.bioperl.org/

=item Wublast 2.0 or higher (required for BLAST search pages)

        http://blast.wustl.edu/

=item BLAT 2.7 or higher (required for BLAST search pages)

	http://www.soe.ucsc.edu/~kent/

=item The MySQL database, version 3.23.39 or higher

	http://www.mysql.com/

=item Modified version of e-PCR (required for e-PCR search page)
...
commands may vary on your system.
...
backslashes at the command line as well.

=head1 PREPARING DIRECTORIES AND USERS

WormBase uses several user accounts for directory and server
permissions.  You will need to create these users and several
preliminary directories.

=head2 User and group accounts
...
Creating a new user and group varies among Unix flavors.  On most
Linux systems, the following commands will create the new groups. You
should have sudo privilege to execute these commands.
...
This command also adds the new acedb user to the acedb group.  Note
that the acedb user's home directory was set to /usr/local/acedb, a
directory which will be created in the next step.
...
=item /usr/local/acedb, owner=acedb group=acedb,mode=drwxrwsr-x
...
=item /usr/local/wormbase, owner=root group=wormbase mode=drwxrwsr-x
...
=item /usr/local/wormbase/logs, owner=root group=wormbase mode=drwxrwsr-x
...
=item ~ftp/pub/wormbase, owner=root group=wormbase mode=drwxrwsr-x
...
You may ignore this step if you do not plan to mirror the WormBase FTP
...
=item /usr/local/wublast, owner=root group=wormbase mode=drwxrwsr-x
...
The "s" bit in the group permissions for these directories ensures
...
=head1 INSTALLING THE WORMBASE FILES

You have two options for installing the WormBase files, by CVS, or
...
use the tarball install.

=over 4

=item Installing from a tarball release
...
Set your CVSROOT environment variable:
 tcsh syntax:
 %setenv CVSROOT :pserver:anonymous@gorgonzola.cshl.org:/usr/local/cvs
...
that you will need to get Acedb up and running.

=head1 INSTALLING ACEDB
...
=item Installing Acedb from a binary package
...
=item Installing Acedb from source
...
Place all the .tar.gz files you need plus INSTALL into this directory
...
=head2 Installing A Preliminary Database And Testing Acedb
...
It will take a while, but it is best to download the current
C. elegans distribution from 

   ftp://ftp.sanger.ac.uk/pub/wormbase/current_release

Download all files into a suitable temp directory, such as /usr/tmp.
Get all the files named database.*.tar.gz, where the * contains the
current data release and software version number.
...
Create the correct release directory and unpack the database tar
files:
...
You will find skeletons of these files in /usr/local/wormbase/wspec,
...
by the WormBase update script to autogenerate new Acedb releases.
...
This means that someone who logs in with username "admin" and a valid
...
You will now use the makeUserPasswd program to create some passwords.
...
confirm that you can communicate with the server:
...
"unknown command" error at this stage.

=head2 Installing Acedb to start automatically
...
is to use inetd to launch the server.

Locate the file /etc/inetd.conf, and add the following line to the
end:
...
Tell inetd to reload its configuration file by sending it a HUP
signal.
...
=head2 Installing Acedb under xinetd

What?  You don't have /etc/inetd.conf?  You are probably using
...
First, make sure that xinetd is even installed (look for the presence
...
The version of xinetd that is installed in RedHat 7.1 will not work.
...
/etc/xinetd.d/acedb with the following contents:
...
Note: to know if the server is listening at port 2005, run the
following command:
...
If an error occurs, check /var/log/messages, and the serverlog.worm
...
assumed to be "me". Substitute for these values as appropriate

=over 4

=item Step 1. Create the elegans database

 mysqladmin -uroot -pPASSWORD create elegans

=item Step 2. Create the elegans_load database

 mysqladmin -uroot -pPASSWORD create elegans_load

=item Step 3. Give yourself write permission for the databases
...
Note that this is set up so that you do not have to type a password to
...
=item Step 4. Give the "nobody" user read permission for the elegans
database
...
All three primary databases will be populated the first time you run
the update_wormbase.pl script. On subsequent updates, the *_load will
become temporarily populated.
...
drive. Otherwise, you should do one of the following:
...
The Ace module is the most recent version of AcePerl.  When you
...
When you install Ace for the first time it will also ask you whether
...
documentation that accompanies the expat library.
...
=head1 INSTALLING BIOPERL

You may install BioPerl either using anonymous CVS or by downloading
and installing the most recent stable core.
...
Installing from CVS will give you the latest version of BioPerl, but
may also include unresolved bugs and experimental code.
...
This will create a directory named bioperl-live.  In the future, when
you wish to update to the most recent version, simply type "cvs
update" in the bioperl-live directory.

Install BioPerl in the usual way, by running "perl Makefile.PL",
"make", "make test" and "make install".  

=head1 INSTALLING GENERIC GENOME BROWSER
...
=head1 INSTALLING APACHE AND MOD_PERL

To a large extent, installing Apache/mod_perl is exactly as described
...
The primary installation site of WormBase also needs the proxy module
and uses this configuration command.
...
The LWP library must be installed before you "make test".  If the
tests are successful, become root and run:

  # make install

This will install Apache into the directory /usr/local/apache.

Note: if you have trouble in installing Apache/mod_perl, you may want to try
older version of there software. Apache_1.3.26 and mod_perl-1.27 work well on 
Red Hat 7.3.

=head1 CONFIGURING WORMBASE

=head2 Configuring httpd.conf
...
If WormBase is going to be the only website hosted by this server,
...
<VirtualHost> section.  Here is a template to follow:
...
The IP address in the <VirtualHost> tag must be replaced by the
...
Do not use "www.wormbase.org!" This name is already taken.
...
=head2 Installing Analog and Report Magic (optional)

If you are running a mirror site or would like to analyze accesses to
your logs, you should also install Analog and ReportMagic.  These
software packages will be used to automatically analyze the access
logs on a running basis.
...
=head2 Configuring WormBase: elegans.pm and localdefs.pm
...
The second file, localdefs.pm, contains site-specific hostnames,
...
=item $ACEPASS, $USERNAME, $PASSWORD

These three items define the acedb username and password.

=item $MYSQL_HOST, $MYSQL_USER, $MYSQL_PASS
...
=item $BLAST2WORMBASE, $WORMBASE2BLAST
...
not work yet because they haven't been generated.

=head2 If it doesn't work

There are a number of common problems to check:

=over 4

=item Is the acedb socket server starting?
...
/usr/local/acedb/elegans/database.  Examine log.wrm and serverlog.wrm.

=item Is the acedb socket server crashing?
...
=item The banner displays but the decorative worm images are broken
...
Jim Kent's BLAT (blast-like alignment tool) is a fast nucleotide
aligner used by the blast search page. If you do not plan to support
blast searches, you may safely skip this step.
...
=head1 INSTALLING SCRIPTS TO VERIFY SERVERS ARE RUNNING
...
Place the restart scripts under cron control of a privileged user.
...
At the same time, you might also wish to automate the rotatation of
...
=head1 THE UPDATE_WORMBASE.PL SCRIPT
...
files used by the web and FTP sites.

=head2 Files touched by update_wormbase.pl
  
   The following files and directories are managed by this script:

=over 4

=item /usr/local/wormbase/mirrored_data
...
=item /usr/local/wormbase/blast_WSXX
...
=item ~ftp/pub/wormbase/elegans, ~ftp/pub/wormbase/briggsae
...
=item The MySQL "elegans", "briggsae", and "elegans_pmap" databases
...
=item /usr/local/wormbase/html/release_notes

This directory is updated with a copy of the current release notes.

=back

=head2 Configuring update_wormbase.pl
...
script is run, or by editing the script directly.
...
Just below the top of the file a line sets the default PATH
...
Below this is a series of constant flags that enable and disable
various steps in the process.  It looks like this:
...
=head2 Running update_wormbase.pl
...
spontaneously, which happens relatively frequently!
...
If you wish to fetch a particular C. elegans release and force it to
be installed, use the -wsversion option.  This is useful for
backtracking to a known working version of the database:

 % update_wormbase.pl -wsversion WS40

=head2 Dealing with Partial Updates
...
the dump files and updating the web site:
...
=head2 Running as a Cron Job

You will probably want to run update_wormbase.pl as a Cron job, using
a crontab line like this one:
...
Material in this document is copyright 2003 by the California
