package OntoSubs;
use CGI qw(:standard *table *TR *td *th escape);
use Exporter 'import';
@EXPORT qw(print_ao_annotations print_search_form print_ontology_choices print_return_choices print_entry_items);
...
sub print_ao_annotations {
    ...
    foreach my $expr_pattern (@expr_patterns) {
        push @unique_ao_terms,
          grep { !$ao_terms_seen{$_}++ } $expr_pattern->Anatomy_term;
    }
    ...
    if ($page_type eq 'ont_gene' ) {
        print TR({},
            th({ 
            -align => 'left', -colspan => 2, -class => 'databody' 
            },'Anatomy Ontology Associations via Expression'
            )
        );
        my $term_width = '30%';
        my $def_width  = '70%';
    ...
    print TR({},
        th({ 
        	-align => 'left', -width => $term_width, -class => 'databody' 
        	},'Term'
        ),
        th({-align => 'left', -width => $def_width, -class => 'databody' 
        	},'Definition'
        )
    );
...
sub print_return_choices {
    ...
    if ($form_type =~ /multi/ ) {
	   print $cgi->start_table({ -border => 1, -cell_spacing => 1, -cellpadding => 1 } );
