...
sub new {
  ...
  #this is for autoconversion of selectors in the selector object
...
sub style {
  ...
  return undef unless $self->source($tag);
...
sub _parse {
  ...
  while($source =~ m/([\w\.].+?)\{(.+?)\}\s*/gs){
	...
	$val =~ s/^['"]//;
	$val =~ s/['"]$//;
        push @{$attribs{$tag}}, [$att,$val] if(defined $att and defined $val); #parse error test
      }    
