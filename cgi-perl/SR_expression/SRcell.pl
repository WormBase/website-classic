...
Script for displaying cell-specific GFP gene expression information. 

	$Author: tharris $
	$Date: 2010-01-25 15:35:54 $
	$Id: SRcell.pl,v 1.1.1.1 2010-01-25 15:35:54 tharris Exp $

Basical behavior: allow users to display gene expression cell by cell.  
...
print
      start_html(-title=>"Cell specific Genes in C. elegans",
      ...
      h2({-style=>'Color: blue;font-size: 150%;font-family: sans-serif'}, "<b>Cell specific gene expression</b>");
...
sub search_form{
  ...
  my @cells = qw(ADF AFD ADL ASE ASG ASH ASI ASJ ASK AWA AWB AWC PHA PHB IL1D IL1 IL2D IL2V IL2);

  print p('Choose a cell for display');

  print " <b>Display genes expressed in </b> ",popup_menu(-name => 'cell_item',
                              -values=>\@cells,
                              -default=>$cells[0] || 'NA',
                              -onLoad   => 'submit()',
                              -onChange => 'submit()'); 
...
sub collect_strain_IDs{
  my $dbh = shift;
  my $cell_param = lc param('cell_item') || 'adf';
  my @strain_ids;
  my $sql_cmd = "SELECT strain_id from detail_observation where $cell_param = 1;";
  ...
  my $strain_id;
  while (($strain_id) = $sth->fetchrow_array){
